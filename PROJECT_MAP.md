...
│   │       ├── CircleElement.tsx
│   │       ├── TextElement.tsx
│   │       ├── TextCenteredElement.tsx # Центрированный текст с векторизацией
...
│   ├── Templates/            # Компоненты шаблонов и генератора
│   │   ├── TemplateCategoryBar.tsx # Панель категорий шаблонов
│   │   ├── TemplateGalleryPage.tsx # Галерея шаблонов
│   │   ├── TemplatePreview.tsx # Превью шаблона
│   │   └── GeneratorPage.tsx # Страница генератора
│   ├── Admin/                # Админ панель
│   │   ├── AdminPanel.tsx    # Основная админ панель
│   │   ├── AdminLoginModal.tsx # Модальное окно входа
│   │   └── AdminTemplatesTab.tsx # Управление шаблонами
...
### 1. Типы данных (types/index.ts и types/templates.ts)
...
### 1.1. Типы шаблонов и генератора (types/templates.ts):
- `TemplateCategory` - категория шаблонов: `id`, `name`, `order`, `isBuiltIn`
- `StampTemplate` - шаблон: `id`, `categoryId`, `name`, `elements`, `canvasSize`, `thumbnail?`, `isBuiltIn`, `createdAt`
- `GeneratorBaseTemplate` - базовый шаблон для генератора: `id`, `name`, `type`, `elements`, `canvasSize`, `iconPosition`, `isBuiltIn`, `createdAt`
- `IconWithTags` - иконка с тегами: `name`, `source`, `displayName`, `tags`, `svgContent?`
- `TemplatesStore` - интерфейс для управления шаблонами
...
### 2.1. Store шаблонов (store/useTemplatesStore.ts)
**Управление категориями:**
- `addCategory()`, `updateCategory()`, `deleteCategory()`, `reorderCategories()`, `getAllCategories()`

**Управление шаблонами:**
- `addTemplate()`, `updateTemplate()`, `deleteTemplate()`, `getTemplatesByCategory()`, `getAllTemplates()`

**Управление генератором:**
- `addGeneratorTemplate()`, `updateGeneratorTemplate()`, `deleteGeneratorTemplate()`, `getGeneratorTemplates()`

**Управление тегами иконок:**
- `setIconTags()`, `getIconTags()`, `searchIconsByTag()`
...
- **LayersPanel** - панель слоёв с drag-and-drop (@dnd-kit):
  ...
  - Визуальные индикаторы для групп и элементов
- **MobileLayersPanel** - адаптированная версия для мобильных устройств
...
- **TextCenteredElement** - центрированный текст с векторизацией
...
- **TemplateCategoryBar** - панель категорий шаблонов
- **TemplateGalleryPage** - галерея шаблонов с фильтрацией
- **TemplatePreview** - предварительный просмотр шаблона
- **GeneratorPage** - страница генератора с поиском по тегам иконок
- **AdminTemplatesTab** - вкладка управления шаблонами в админ панели
...
### ✅ Система шаблонов и генератора (ноябрь 2025)
- **Управление категориями шаблонов** - создание, редактирование, удаление пользовательских категорий
- **Система шаблонов** - сохранение и загрузка пользовательских шаблонов с привязкой к категориям
- **Генератор штампов** - создание штампов на основе базовых шаблонов и поиска иконок по тегам
- **Базовые шаблоны для генератора** - шаблоны с указанием позиции для вставки иконки
- **Поиск иконок по тегам** - интеллектуальный поиск иконок по ключевым словам
- **Админ панель шаблонов** - удобное управление шаблонами и категориями
- **Встроенные категории** - ИП, ООО, медицинские, самозанятые
- **Типы генераторов** - поддержка различных типов штампов (ип, ооо, медицина, самозанятые и др.)

### ✅ Улучшенная векторизация текста (ноябрь 2025)
- **Векторизация кривого текста** - текст по кругу сохраняется как векторные пути (useCurvedTextVectorization)
- **Векторизация центрированного текста** - прямой текст сохраняется как векторные пути (useCenteredTextVectorization)
- **Полная векторизация** - все текстовые элементы экспортируются как векторы, сохраняя шрифты
- **useGoogleFonts хук** - динамическая загрузка Google Fonts для корректной векторизации
- **Лучшая совместимость** - правильно отображаются шрифты в PDF, PNG, SVG экспортёрах
- **Поддержка кириллицы** - корректная обработка кириллических символов в шрифтах

### ✅ Управление слоями (январь 2025)
- **Drag-and-drop переупорядочивание** - элементы можно перетаскивать для изменения порядка отображения
- **Иерархическое отображение** - группы элементов отображаются в виде дерева
- **Управление видимостью** - кнопки показать/скрыть элементы
- **Блокировка элементов** - возможность заблокировать элементы от редактирования
- **Множественный выбор** - группировка нескольких элементов одновременно
- **Кнопки управления порядком** - быстрое перемещение элементов (вперёд, назад, на передний/задний план)
- **MobileLayersPanel** - мобильная версия панели слоёв для адаптивного интерфейса
- **Интеграция с группировкой** - слои работают вместе с системой групп элементов
